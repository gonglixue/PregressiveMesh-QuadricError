...
void Simplification::InitSimplification(Mesh *mesh_in)
{
	...
	printf("init simplification done.\n");
}
...
void Simplification::AssignInitialQ()
{
    ...
    for(VertexIter vi = mesh->vertices.begin(); vi != mesh->vertices.end(); vi++){
        ...
        if(vi->isBoundary == false) startHalfEdge = vi->neighborHe;  // 以vi为起点的（一条）halfEdge
        else                        startHalfEdge = FindBoundaryEdgeIncidentToVertexInCW(vi->neighborHe);  //?

        HalfEdge *hep = startHalfEdge;
		// 遍历与该顶点关联的每条halfEdge对应的面，用面法线计算Q
        do{
            CumulateQ(vi, hep->face->normal, -DotProduct(hep->face->normal, hep->face->halfedge[0].vertex->coord)); //累加到vi->Q
...
{
    VertexIter v0 = ei->halfedge[0]->vertex;  // 边的端点
    VertexIter v1 = ei->halfedge[0]->next->vertex;

    double newQ[4][4];  // 收缩后产生新点的Q = Q1+Q2
    ...
    for(int i = 0; i < 3; i++) for(int j = 0; j < 4; j++) matrix[i][j] = newQ[i][j];  // matrix：Q重置最后一行
    ...
    if( SolveLinearSystem(matrix, rhs, solution) ){
        ...
        for(int i = 0; i < 3; i++) optimalCoord[i] = solution[i];  // 收缩后的新点的坐标
...
bool Simplification::EdgeCollapse()
{
    ...
    while(heap.empty() == false){
        ...
        if(ect.ei->isActive == true && ect.id == ect.ei->ect_id) { // ect.id==ect.ei->ect_id?
